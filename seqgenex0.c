...
void main(void)
{
    ...
    // Print uname -a output to syslog
    fp = popen("uname -a", "r");
    if (fp != NULL) {
        if (fgets(uname_buffer, sizeof(uname_buffer), fp) != NULL) {
            syslog(LOG_CRIT, "%s", uname_buffer);
        }
        pclose(fp);
    }
...
{
    double current_time, start_service;
    ...
    while(!abortS1)
    {
        ...
        start_service=getTimeMsec();
        unsigned long long result = fib(39);
        current_time=getTimeMsec();

        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:2] Thread 1 start X @%lf on core %d\n", start_service, sched_getcpu());
        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:2] Thread 1 end %llu @%lf on core %d\n", result, current_time, sched_getcpu());
    }
...
void *Service_2(void *threadp)
{
    ...
    while(!abortS2)
    {
        ...
        start_service=getTimeMsec();
        unsigned long long result = fib(37);
        current_time=getTimeMsec();

        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:2] Thread 2 start X @%lf on core %d\n", start_service, sched_getcpu());
        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:2] Thread 2 end %llu @%lf on core %d\n", result, current_time, sched_getcpu());
    }
...
void *Service_3(void *threadp)
{
    ...
    while(!abortS3)
    {
        ...
        start_service=getTimeMsec();
        unsigned long long result = fib(40);
        current_time=getTimeMsec();

        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:2] Thread 3 start X @%lf on core %d\n", start_service, sched_getcpu());
        syslog(LOG_CRIT, "[COURSE:1][ASSIGNMENT:2] Thread 3 end %llu @%lf on core %d\n", result, current_time, sched_getcpu());
    }
